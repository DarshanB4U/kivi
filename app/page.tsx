...
import { BookOpen, Users, ArrowRight, Sparkles, PlayCircle } from "lucide-react";
        ...
        <div className="max-w-7xl mx-auto flex h-16 items-center justify-between px-6">
          <div className="flex items-center gap-2 group cursor-pointer">
             <div className="group-hover:scale-110 transition-transform duration-300">
                <img src="/kiwi-svgrepo-com.svg" alt="Kivi Logo" className="w-8 h-8" />
             </div>
             <span className="font-black text-2xl tracking-tighter hover:text-primary transition-colors">Kivi</span>
          </div>
          <div className="flex items-center gap-4">
            <Button variant="ghost" className="hidden sm:flex font-medium hover:bg-primary/10" asChild>
              <Link href="/signin">Sign in</Link>
            </Button>
            <Button className="rounded-full px-6 shadow-xl shadow-primary/20 hover:shadow-primary/40 hover:-translate-y-0.5 transition-all duration-300 font-semibold" asChild>
      ...
      <section className="relative pt-32 pb-24 px-6 overflow-hidden">
        <div className="max-w-5xl mx-auto text-center space-y-10 relative z-10">
          ...
          <p className="text-xl md:text-2xl text-muted-foreground max-w-2xl mx-auto font-medium leading-relaxed animate-in fade-in slide-in-from-bottom-10 duration-1000 delay-300">
            The elite ecosystem for creators to build world-class courses and students to master the skills of tomorrow.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-5 pt-10 animate-in fade-in slide-in-from-bottom-12 duration-1000 delay-500">
            <Button size="lg" className="rounded-full px-10 h-16 text-lg shadow-2xl shadow-primary/30 hover:shadow-primary/50 transition-all hover:-translate-y-1.5 active:scale-95 w-full sm:w-auto group font-bold" asChild>
              <Link href="/signup">
                Start building now <ArrowRight className="ml-2 w-5 h-5 group-hover:translate-x-1.5 transition-transform" />
              </Link>
            </Button>
            <Button variant="outline" size="lg" className="rounded-full px-10 h-16 text-lg border-2 hover:bg-muted/50 w-full sm:w-auto group font-bold backdrop-blur-sm transition-all" asChild>
              <Link href="#courses">
        ...
        <div className="max-w-7xl mx-auto space-y-20">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mr-6">
            <div className="space-y-4">
              <h2 className="text-4xl md:text-6xl font-black tracking-tighter">Featured Masterclasses</h2>
              <p className="text-xl text-muted-foreground max-w-xl font-medium">Curated high-performance content from world-class industry leaders.</p>
            </div>
          ...
          {courses.length === 0 ? (
            <Card className="border-dashed bg-background/50 backdrop-blur-sm border-2 rounded-[2rem]">
              <CardContent className="py-32 text-center text-muted-foreground flex flex-col items-center gap-6">
                ...
                <div className="space-y-2">
                  <p className="text-2xl font-bold text-foreground">No courses available yet.</p>
                  <p className="text-lg">The next big thing is currently being built.</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
              {courses.map((course) => (
                <Card key={course.id} className="flex flex-col group hover:shadow-[0_40px_80px_-15px_rgba(0,0,0,0.1)] dark:hover:shadow-[0_40px_80px_-15px_rgba(0,0,0,0.3)] transition-all duration-500 border-none overflow-hidden glass-card rounded-[2rem] hover:-translate-y-2">
                  <div className="aspect-[16/10] bg-muted relative overflow-hidden m-4 mb-0 rounded-2xl">
                    {course.thumbnail ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center bg-linear-to-br from-muted to-muted/50 group-hover:scale-110 transition-transform duration-700 ease-out">
                        <BookOpen className="text-muted-foreground/20" size={64} />
                      </div>
                    )}
                  ...
                  <CardHeader className="flex-1 px-8 pt-8">
                    <div className="flex items-center gap-3 mb-4">
                      <Badge variant="secondary" className="text-xs font-bold uppercase tracking-wider rounded-lg px-2.5 py-1 bg-primary/10 text-primary border-none">
                    ...
                    <CardTitle className="text-2xl font-bold tracking-tight line-clamp-2 leading-tight group-hover:text-primary transition-colors mb-4">{course.title}</CardTitle>
                    <CardDescription className="line-clamp-2 text-base font-medium leading-relaxed text-muted-foreground/80">{course.description}</CardDescription>
                  </CardHeader>
                  ...
                  <CardFooter className="px-8 pb-8 pt-0 mt-4 h-20 flex items-center justify-between border-t border-primary/5">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col -space-y-1">
                        <span className="text-xs font-bold text-muted-foreground uppercase tracking-tighter">Creator</span>
                        <span className="text-sm font-bold truncate max-w-[140px]">
                          {course.creator.name || course.creator.email.split('@')[0]}
                        </span>
                      </div>
                    </div>
                    <Button size="icon" variant="ghost" className="rounded-full bg-primary/5 hover:bg-primary hover:text-white transition-all group/btn">
